...
| 5 | 19:30Z | Warn | Game - Stats | Reported by David live (OSU vs Illinois): once the header scrolls away, the Team stats card is two columns of numbers with no team names - you can't tell whose is whose. Screen readers get no names either. Proposed: a header row in the card with each team's abbreviation (in its colour), kept in view while the card scrolls. | Proposed - awaiting David |
