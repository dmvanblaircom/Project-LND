...
| C18 | **Matchup card: yards allowed without CFBD** | M | CFBD declined on 2026-09-25 to allow its figures as public JSON (a follow-up asking what it *would* allow is drafted in David's Gmail). Free route: add up opponents' rushing and passing yards from ESPN's box score for each game already played, computed in the app. ESPN data only; nothing republished from CFBD. Revisit CFBD if Bill's answer opens a path. |
