...
| `stage/team-data` | C2: team files in `data/<team id>/`, Kalshi markets in `data/league/`; producers write where the team's config says; `odds.yml` loops over the teams that declare each kind; odds history and news stamped with their team; new `tools/pipelinecheck.py` in CI. `VERSION` `suite-2026-09-26d` | All 26 gates pass. Merging: main's data commits carry into the new folders by rename (tested against main at 6317055). If git ever reports modify/delete on a root data file, take main's version into `data/<team>/` and delete the root file. Merge between data runs: a run already under way when this lands fails its push once, harmlessly. |
